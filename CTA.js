...
function mapVolunteersToRoles(volunteers, nextSundayDate) {
    const roles = {
        ...
        singer: 'No Volunteer Set in Elvanto',
    ...
    volunteers.forEach(vol => {
        if (vol.volunteers != "") {
            ...
            for (var x = 0; x < vol.volunteers.volunteer.length; x++) {
                volunteerNames.push(
                    `${vol.volunteers.volunteer[x].person.firstname} ${vol.volunteers.volunteer[x].person.lastname}`
                );
            }

            volunteerNames = volunteerNames.join(" & ");

            if (vol.position_name === 'Acoustic Guitar') roles.acousticGuitar = volunteerNames; //SERVICE
            else if (vol.position_name === 'Worship Leader') roles.worshipLeader = volunteerNames;
            else if (vol.position_name === 'Singer') roles.singer = volunteerNames;
            else if (vol.position_name === 'Electric Guitar') roles.electricGuitar = volunteerNames;
            else if (vol.position_name === 'Bass Guitar') roles.bassGuitar = volunteerNames;
            else if (vol.position_name === 'Drum') roles.drummer = volunteerNames;
            else if (vol.position_name === 'Keyboard') roles.keyboardist = volunteerNames;
            else if (vol.position_name === 'Sound Engineer') roles.soundEngineer = volunteerNames;
            else if (vol.position_name === 'SSM In Charge') roles.sundayServiceManager = volunteerNames;
            else if (vol.position_name === 'Music Director (MD)') roles.musicDirector = volunteerNames;
            else if (vol.position_name === 'Multimedia Operator') roles.multimediaOperator = volunteerNames; //MULTIMEDIA
            else if (vol.position_name === 'Livestream Operator') roles.livestreamOperator = volunteerNames;
            else if (vol.position_name === 'Offering Speaker') roles.offeringSpeaker = volunteerNames; //DISCIPLESHIP
            else if (vol.position_name === 'Preacher') roles.preacher = volunteerNames; //PREACHER
        }
    });
